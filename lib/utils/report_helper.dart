...
class ReportHelper {
                      ...
                      onTap: () async {
                        final d = await showDatePicker(
                          ...
                          builder: (context, child) =>
                              _buildPickerTheme(context, child, themeColor),
                        );
                        if (d != null)
                          setModalState(
  ...
  static Future<DateTime?> showAppDatePicker(
    ...
    final surfaceColor = isDark ? const Color(0xFF1A1D2D) : Colors.white;
    final textColor = isDark ? Colors.white : const Color(0xFF2D3436);
    // Secondary text color is available for potential future use
  ...
  static Widget _buildPickerTheme(
    BuildContext context,
    Widget? child,
    Color themeColor,
  ) {
  ...
  static Future<void> generateExcel(
    ...
    try {
      ...
      final fileName =
          "Business_Report_${DateTime.now().millisecondsSinceEpoch}.xlsx";
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Error saving Excel: $e. Try checking storage permissions.',
            ),
            backgroundColor: Colors.red,
          ),
        );
  ...
  static String getCleanItems(TransactionModel tx) {
    ...
    return items
        .map((i) {
          ...
          String suffix = i['variant']?.toLowerCase() == 'half'
              ? " (Half)"
              : "";
