...
class _CategoryManagementScreenState extends State<CategoryManagementScreen> with SingleTickerProviderStateMixin {
              ...
              return Container(
                ...
                child: ListTile(
                  ...
                  leading: Row(
                    ...
                    children: [
                      Icon(Icons.drag_handle, color: profile.secondaryTextColor.withValues(alpha: 0.3), size: 20),
                      const SizedBox(width: 12),
                      Container(
                        padding: const EdgeInsets.all(10),
                        decoration: BoxDecoration(
                          color: (cat.type == 'selling' ? Colors.green : (cat.type == 'readymade' ? Colors.blue : Colors.orange)).withValues(alpha: 0.1),
                          shape: BoxShape.circle,
                        ),
                        child: Icon(_getIconData(cat.iconName), color: cat.type == 'selling' ? Colors.green : (cat.type == 'readymade' ? Colors.blue : Colors.orange), size: 22),
                      ),
                    ],
                  ),
                  title: Text(
                    ...
                    child: Wrap(
                      ...
                      children: [
                        Container(
                          ...
                          child: Text(
                            cat.type == 'selling' ? 'Selling' : (cat.type == 'readymade' ? 'Readymade' : 'Purchase'),
                            style: TextStyle(fontSize: 10, color: cat.type == 'selling' ? Colors.green : (cat.type == 'readymade' ? Colors.blue : Colors.orange), fontWeight: FontWeight.bold),
                          ),
                        ),
  ...
  void _showCategoryBottomSheet(BuildContext context, CategoryProvider provider, ProfileProvider profile, {CategoryModel? category}) {
    ...
    final stockController = TextEditingController(text: category?.stockQty.toStringAsFixed(0) ?? '0');
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                            ...
                            child: GestureDetector(
                              ...
                              child: AnimatedContainer(
                                ...
                                child: Text('PURCHASE', textAlign: TextAlign.center, 
                                  style: TextStyle(color: selectedType == 'purchase' ? Colors.white : profile.secondaryTextColor, fontWeight: FontWeight.w900, fontSize: 11)),
                              ),
                            ),
                          ),
                          const SizedBox(width: 8),
                          Expanded(
                            child: GestureDetector(
                              onTap: () => setModalState(() => selectedType = 'readymade'),
                              child: AnimatedContainer(
                                duration: const Duration(milliseconds: 200),
                                padding: const EdgeInsets.symmetric(vertical: 14),
                                decoration: BoxDecoration(
                                  color: selectedType == 'readymade' ? Colors.blue : profile.scaffoldColor,
                                  borderRadius: BorderRadius.circular(16),
                                  border: Border.all(color: selectedType == 'readymade' ? Colors.blue : (profile.isDarkMode ? Colors.white10 : Colors.grey.shade200)),
                                ),
                                child: Text('READYMADE', textAlign: TextAlign.center, 
                                  style: TextStyle(color: selectedType == 'readymade' ? Colors.white : profile.secondaryTextColor, fontWeight: FontWeight.w900, fontSize: 11)),
                              ),
                            ),
                          ),
                        ],
                      ...
                      if (selectedType == 'selling' || selectedType == 'readymade') ...[
                        Container(
                          padding: const EdgeInsets.all(16),
                          decoration: BoxDecoration(
                            color: (selectedType == 'readymade' ? Colors.blue : Colors.green).withValues(alpha: 0.05),
                            borderRadius: BorderRadius.circular(20),
                            border: Border.all(color: (selectedType == 'readymade' ? Colors.blue : Colors.green).withValues(alpha: 0.1)),
                          ),
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Row(
                                ...
                                children: [
                                  Row(
                                    children: [
                                      Icon(Icons.inventory_2_outlined, color: (selectedType == 'readymade' ? Colors.blue : Colors.green), size: 20),
                                      const SizedBox(width: 12),
                                  ...
                                  Switch(
                                    ...
                                    activeColor: (selectedType == 'readymade' ? Colors.blue : Colors.green),
                              ...
                              if (useCategoryStock) ...[
                                ...
                                const Text('Items in this category will share this stock pool and sync automatically.', 
  ...
  void _showDeleteWarning(BuildContext context, CategoryProvider provider, ProfileProvider profile, CategoryModel category) async {
    final confirm = await AppBottomSheet.showAction(
      context: context,
      profile: profile,
      title: 'Delete Category?',
      message: 'Are you sure you want to delete "${category.name}"? All items in this category will be moved to another category.',
      confirmLabel: 'DELETE',
      isDestructive: true,
